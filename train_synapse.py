...
# 执行链为：解析命令行 -> 固定随机性 -> 整理数据集配置 -> 生成实验目录名 ->
# 创建 EMCADNet -> 把模型放到 GPU -> 调用 trainer.py 中的 trainer_synapse。
# 论文对应关系：网络整体见仓库论文第 3.2 节及图 2；多阶段输出监督见第 3.3 节；
# Synapse 的 224x224、300 epoch、batch size 6、AdamW、1e-4 学习率/权重衰减、
# 0.3*CrossEntropy+0.7*Dice 等设置见论文第 4.1 节 Implementation details。
# 这里的 num_classes=9 表示“背景 1 类 + 8 个前景器官”，不是 9 个前景器官。
# 典型训练张量：输入 image=[B,1,224,224]，标签 label=[B,224,224]；
# EMCADNet 内部先把单通道 CT 映射为 3 通道，再输出 4 个 [B,9,224,224] logits。
# ========================================================================
